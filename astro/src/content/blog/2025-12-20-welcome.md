...
**Figure 3.**  Effect of learning-rate warm-up, learning rate, and initialization $\alpha$ on training stability in ViT-B (12 layers, $N=768$) on CIFAR-100 for $\text{DyT}$ and pre-LN. **(a)** Warm-up sweep for $\text{DyT}$. **(b)** Learning-rate sweep comparing $\text{DyT}$ ($\alpha=1$) to pre-LN without warm-up. **(c)** Test accuracy at epoch 9 versus $\alpha$ and warm-up epochs. **(d)** Accuracy versus $\alpha$. Overall, $\text{DyT}$ is more sensitive to warm-up and learning-rate choices, especially at larger $\alpha$.
