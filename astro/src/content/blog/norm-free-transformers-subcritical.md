...
Normalization-free Transformers that replace LayerNorm with saturating pointwise functions (DyT/Derf) trade computational simplicity for worse signal propagation across layers. In the mean-field picture, pre-LN residual networks are effectively critical at initialization: gradient amplification grows only as a power law with depth. By contrast, $\tanh$-like normalization functions are subcritical in residual networks: they induce stretched-exponential gradient amplification, which can make optimization more sensitive to hyperparameters.
